...
public class ISAXIndex implements java.io.Serializable, Iterable<Long> {
	...
	protected final int minCap;
	protected final int maxCap;
	protected final int maxWidth;
	protected final int cardinality;
	public final Distance df;

	protected static final Logger logger = Logger.getLogger(ISAXIndex.class
			.getName());
	...
	public Long[] getOccurence(String pattern) {
		ISAX o = new ISAX(pattern, dimension, cardinality);
		Node startNode = root;
		...
		if (path.peek().isLeaf()) {
			Leaf l = (Leaf) path.peek();
			return l.children.toArray(new Long[l.numChildren()]);
		} else {
			return new Long[0];
		}
	}
	...
	public Iterator<Leaf> leafIterator() {
		return new BreadthFirstLeafSearch(root);
	}
	...
	class BreadthFirstLeafSearch implements Iterator<Leaf> {

		// Implement BFS with Iterative deepening depth-first search (IDDFS)
		// method
		...
		private BreadthFirstLeafSearch(Node n) {
			start = n;
			...
			while (depth < maxWidth + 2) {
				if (path.isEmpty()) {
					...
					if (path.peek().isLeaf() && path.size() == depth) {
						next = (Leaf) path.peek();
						logger.finer("found leaf " + next.dispLoad()
								+ " at depth " + path.size());
						return true;
